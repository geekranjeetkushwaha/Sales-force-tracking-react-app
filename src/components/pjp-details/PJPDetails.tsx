...
import type { PJPDetails as PJPDetailsData } from '../../types/pjp';

interface PJPDetailsProps {
  isOpen: boolean;
  onClose: () => void;
  onStartVisit?: (item: PJPDetailsData) => void;
  data: PJPDetailsData | null;
  title?: string;
}
    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h3 className="text-xl font-bold text-gray-800 mb-1">{data.name || 'N/A'}</h3>
              <p className="text-gray-600 text-sm font-medium">Code: {data.code || 'N/A'}</p>
              {data.status && (
                <div className="mt-2">
                  <Tag
            ...
            {data.visitCount !== undefined && (
              <div className="text-right">
                <div className="text-2xl font-bold text-blue-600">{data.visitCount}</div>
        ...
        >
          ...
          {data.region && <Descriptions.Item label="Region">{data.region}</Descriptions.Item>}
        </Descriptions>
        ...
        {data.notes && (
          <>
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm bg-gray-50 p-3 rounded border">{data.notes}</p>
            </div>
          </>
        )}
